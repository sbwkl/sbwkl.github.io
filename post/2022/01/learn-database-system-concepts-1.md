...
B+ 数每个非叶子节点最大的扇出需要相同，如果搜索码是数字类型，这个很容易，数字类型占用的空间相同。如果是字符串类型，因为字符串长度可变就难以控制占用空间，为此有一个办法是固定取字符串前 m 个字符，这样就可以保证每个节点最大扇出相同。
...
这种索引在交集很小的时候特别有用，比如查询男性 VIP3 的用户数，这个时候只要拿男性位图和 VIP3 位图做按位与操作就能拿到结果。再者计算机的一次位操作就是 32 或者 64 比特，效率会很高。
