...
意思是左边的 stmt 可以用右边的形式来表示。

这里面的三个 stmt 虽然拼写一样，但表达的内容不同，可以类比成类的 3 个对象，有时用下标区别。

在这个式子里，stmt 叫做非终结符号，if ( ) else 叫做终结符号，在左边出现的都是非终结符。
...
从开始符号开始不断代入产生式，所能产生的所有终结符号串的集合叫做这个文法的语言，因吹斯听。
...
就会有二义性，还是这个表达式：9 - 5 + 2，有两棵分析树可以推导出
...
根据小学数学，乘除的优先级要高于加减，文法的产生式是这么设计
