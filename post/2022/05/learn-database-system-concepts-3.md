...
事务是访问或者更新数据的程序执行单元，具有四个特性：原子性、隔离性、持久性、一致性，合起来就是传说中的 ACID 八股文一般都背过。

并发控制系统 (Concurrency Control) 保证隔离性，恢复系统 (Recovery System) 保证原子性和持久性，AID 三个特性有了，一致性也有了。

还有一种一致性理解是提交的数据需要满足所有定义的规则，比如约束（主键约束等），这个有些是数据库能保证的，有些需要开发人员保证。比如转账操作，需要保持转账前和转账后总金额不变。

并发控制系统实现隔离性，这个隔离性我还是觉得别扭，总觉得叫并发性更好理解。强行理解的话那就是一个事务对数据的变更，另一个事务不一定看得到，这样就算隔离了。

隔离性有 4 个隔离等级，可串行化（Serializable），可重复读（Repeatable read），已提交读（Read committed），未提交读（Read uncommitted）。从前往后并发度越来越高，性能越来越好。

一个事务可以抽象为两种基本操作 read(X) 和 write(X)，其实还有一个很重要的 commit 操作。


+ 调度（schedules）：事务读写操作执行顺序，可以设计多个事务，事务之间也能交叉执行。
+ 串行（serial）：事务的读写操作按照事务顺序一个一个执行。
+ 可串行化调度（serializable schedules）：虽然读写顺序是交叉运行的，但是效果和串行一样的调度。
...
要找这样的调度，需要等价规则，类似关系代数。那么哪里才能找到等价规则呢，刚好这里就有个冲突等价（conflict equivalent）。

两个事务操作相同的数据项 X 且至少有一个写操作，那么我们说两个事务冲突（conflict）。冲突的指令不能交换顺序，但是不冲突的执行可以。

比如调度 2 里 T1 的 read(B) 和 T2 的 write(A) 不冲突，那么就可以交换顺序。

一个调度通过交换不冲突指令生成另一个调度，那么这两个调度冲突等价（conflict equivalent），这个也有点别扭，明明是交换不冲突指令，却叫冲突等价，随它去了，能理解就好。

一个调度经过一操子不冲突交换，最终与一个串行调度等价，那么这个调度是冲突可串行化（conflict serializable），名字越来越离谱了。

比如调度 2，把 T1 的 read(B) 和 write(B) 一直往上移就能得到调度 1 那么调度 2 就是冲突可串行化。

用优先图（precedence graph）侦测调度是否冲突可串行化，图论无处不在。
...
除了冲突等价还有别的等价，我就没去了解了。

实现并发控制有几种方式：锁、时间戳、多版本。

<!-- -->



+ 可串行化：一组事务虽然并发执行，但是结果和事务串行执行是一致的。其实交叉执行的主要是每个事务里的 read/write 操作
