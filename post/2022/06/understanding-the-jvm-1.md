...
这本书朋友推荐挺久了，一直没有看，最近几周抽空看完了，收获不少。

**增加信心，这几年的工作没白干**

从这本书里面找到了自信，为什么这么说，因为这本书看出了查漏补缺的感觉。不同于之前看的几本书，那不是看自己缺什么，而是看自己有什么，看的怀疑人生。

这说明虽然大部分工作在重复，还是有一些在积累。

**调优本地开发环境**

以前说起 jvm 调优，眼睛盯着服务器，然后发现没啥好调，也不太敢调。

其实作为一个 Java 开发，本地一直有很多进程可以调，属实灯下黑了，以前咋就没注意捏。

本地的开发环境还真有点问题，内存不足，有几次都闪退了，升级到了 16G 还是缺。
...
莫名变多的 Java 进程是 gradle 启动的，经过一番搜索之后，发现是配置问题

![](./gradle-1.png)

上图中红框位置选择 Intellij IDEA 就解决了，奇怪的是之前 2018 版好像没这个问题。

而且 gradle 也真是，都叫 gradle daemon 进程了，每次启动都创建，真的不是 bug 么。


然后是 idea 问题，通过 visualVM 查看 jvm 参数。看到几个参数 -Xmx2048m -XX:+UseG1GC。G1 是高性能的垃圾收集器，相对的内存占用会多一点，所以我换了一个 CMS。
...
方法区本来想给 512m 的，但又小概率会超出，然后疯狂 GC 又 GC 不掉东西，直接寄了。

实话说 idea 非堆内存占用比堆内存还多，真的牛批，真不知道往里面塞了什么东西。

然后给开发中的项目也限制了下内存，观察了下项目占用内存不多，给个 300m 就够了。

加了这些参数后至少内存占用不会到 99% 了。

这次折腾实际也没解决多少问题，多了解几个工具怎么用。另外 visualVM 最好把所有插件都装上，不然功能有点寒酸。

**了解 Class 文件是个什么东西**

跟着书中的例子，把字节码文件从头读了一遍，读前读后明显感觉看 Class 文件的眼神不一样了。

看懂 Class 文件，需要频繁查阅《Java 虚拟机规范》，这个应该算是 Java 虚拟机的需求文档吧。
...
开头 4 个字节固定为 0xCAFEBABE，这是一个魔数，表示这是一个 Class 文件，这个魔数似乎也解释了为啥 Java 的图标是杯咖啡。

接下来 4 个字节是次版本号和主版本号，分别是 0x0000 和 0x0044 换成十进制是 0 和 52 也就是反编译的时候看的 `Decompiled .class file, bytecode version: 52.0 (Java 8)`。


然后是常量池，头两个字节表示数量 0x44 转成十进制是 68 代表的是有 1~67 个常量。

这个挺反常的，光是索引是从 1 开始的就很奇怪了，68 表示的是 67 个常量更诡异，怕不是把索引 0 也算进去了。

67 个常量池读完到 0x000000FF 占了大半的空间了。
...
按照规范读是：一个属性，属性名是 #16 常量（SourceFile），属性值是 #17 常量。

属性格式最复杂，而且能嵌套，能出现在多种结构里。

根据指向常量的值来确定属性名，比如这个属性是 SourceFile，后面还有 Code 属性，可以任意扩展。

前六个字节是固定的，两个指向常量，四个表示长度，之后的内容根据属性名对应的格式读。
...
第一个方法（0x00000114 开始），public 修饰，方法名是 #7 常量，描述符是 #8 常量，一个属性，属性值是 #9 常量，也就是 Code，长度是 0x2F 个字节（三行少一个，到 0x00000150）。

第二个方法（0x00000151 开始），public 修饰，方法名是 #14 常量，描述符是 #15 常量，一个属性，属性值是 #9 长度是 0x31 个字节（三行多一个，到 0x0000018F）
...
两个方法都有 Code 属性，代表方法体实现，按照规范格式如下。
...
Java 字节码还有自己的指令集还是蛮震惊的，它这是想给 CPU 指令做一层统一的规范么。

Class 文件和 gcc 编译过程中产生的 Relocatable object file 有点像，都是半成品。

Class 文件被加载到内存之后才进行 Link 阶段，组合能运行的程序，这灵活性确实很大了。
...
第二次是 Hotspot 虚拟机的名字的来源，指的是虚拟机会把执行的比较多的字节码（热点代码）编译成机器码以提高运行效率，用的是常常听说的 JIT 的编译器。

比较神奇的是 Java 还有提前编译器（AOT）直接把静态代码编译成机器码，这……万物皆轮回么。

另外好消息是编译用的 javac 居然使用 Java 写的，最先进的 Graal 编译器也是用 Java 写的。

**泛型居然是技术债**

Java 的泛型擦除是综合各方面因素之后的妥协，用了最简单的实现，直接擦除掉泛型类型，这样就能兼容之前版本的字节码了，然后增加类似强转的功能，成了。
