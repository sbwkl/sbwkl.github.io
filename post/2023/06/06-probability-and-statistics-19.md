...
看到这段我发现 day 17 的文章有错误，倒数 3，4 两个公式漏了前提条件 $X_i$ 独立，如果不独立的话应该是
...
所以 $Var(X+Y)=Var(X) + Var(Y) + 2Cov(X,Y)$，回到 day 17 的投资组合题，如果 $\rho(R_1, R_2) = -0.3$ 那么

$$
Var(s_1R_1 + s_2R_2 + 0.036s_3) = 55s_1^2 + 28s_2^2 - 0.6\sqrt{55*28}s_1s_2
$$

同样期望下，这个方差比 $R_1, R_2$ 独立时更低，组合选取相关度低的指数。

这应该是鸡蛋不要放在同一个篮子里的数学解释。

之前看到过一张图，大部分指数之间正相关，国债和原油的相关度低一些。
